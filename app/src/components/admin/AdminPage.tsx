...
import { useConfiguracion } from '@/hooks/useValoraciones'
import {
...
export default function AdminPage() {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ id, perfilId }: { id: string; perfilId: number }) => {
      const { error } = await supabase.from('profiles').update({ perfil_id: perfilId }).eq('id', id)
      if (error) throw error
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profiles'] })
                    ...
                    <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-2.5 pr-4">
                        <select
                          ...
                          onChange={(e) =>
                            updateRoleMutation.mutate({ id: u.id, perfilId: Number(e.target.value) })
                          }
                      ...
                      <td className="py-2.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            onClick={() =>
                              setDialog({ mode: 'reset_password', userId: u.id, userName: u.nombres ?? u.email })
                            }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-sm text-slate-500 mb-4">
              ¿Eliminar a <strong>{deleteConfirm.nombres ?? deleteConfirm.email}</strong>?
